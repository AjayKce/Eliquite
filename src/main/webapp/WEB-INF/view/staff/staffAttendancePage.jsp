		...
		<div class="container-fluid">
			...
			<div class="collapse navbar-collapse" id="myNavbar">
				<ul class="nav navbar-nav">
					<li><a href="${pageContext.request.contextPath}/staffClass"><span
							class="glyphicon glyphicon-blackboard"></span> classes</a></li>
					<li><a
						href="${pageContext.request.contextPath}/subjectsHandled"><span
							class="glyphicon glyphicon-education"></span> subject handled</a></li>
					<li class="activeTab"><a
						href="${pageContext.request.contextPath}/staffAttendancePage"><span
							class="fa fa-exclamation"></span> Attendance</a></li>
				</ul>
				<ul class="nav navbar-nav navbar-right">
					<li><a><span class="glyphicon glyphicon-user"></span>
							Hello,<%=session.getAttribute("user")%></a></li>
					<li><a href="${pageContext.request.contextPath}/staffLogOut"><span
						...
						<td>
							<form action="attendanceList" method="post"
								...
								<input type="hidden" name="semester"
									value="<%=temp.getSemester()%>" /> <input type="hidden"
									name="section" value="<%=temp.getSection()%>" /> <input
									type="hidden" name="subjectCode"
									value="<%=temp.getSubjectCode()%>" />
								<button type="submit" class="btn btn-default">
									<span class="glyphicon glyphicon-tag"></span> Set Attendance
								</button>
							</form>
						</td>
						<td>
							<form action="staffPercentageAttendanceRecord" method="post"	style="margin: 0px; padding: 0px; border: 0px;">
								<input type="hidden" name="batch" value="<%=temp.getBatch()%>" />
								...
								<button type="submit" class="btn btn-default form-control form-control-sm">
									<span class="fa fa-book"></span> Report
								</button>
  								</div>
							</form>
						</td>
						<td>
							<form target="_blank" action="staffAttendanceRecord" method="post"	style="margin: 0px; padding: 0px; border: 0px;">
								<input type="hidden" name="batch" value="<%=temp.getBatch()%>" />
								<input type="hidden" name="semester" value="<%=temp.getSemester()%>" />
								<input type="hidden" name="section" value="<%=temp.getSection()%>" />
								<input type="hidden" name="subjectCode"	value="<%=temp.getSubjectCode()%>" />
